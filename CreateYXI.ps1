...
foreach ($sourceFolder in $folders) {
    ...
    Get-ChildItem $sourceFolder -Exclude '*.bat', '*.ps1', '.*', '*.bak', 'Test*.yxmd', 'README.md' | Copy-Item -Destination "$temp\$leaf" -Recurse
}

Push-Location $temp
# Compress-Archive -Path "$temp" -DestinationPath "$name.zip" -Verbose -Update
7z a -tzip "$name.zip" *
Pop-Location
